...
export default function Home() {
  ...
  const [billType, setBillType]         = useState(null); // 'gst' | 'nongst'
  ...
  const buildBill = () => {
    const filled = rows.filter(r => r.particulars?.trim() && parseFloat(r.rate) > 0);
    if (filled.length === 0) {
      alert('Fill at least one row (particulars and rate required).');
      return null;
    }
    const billNumber = confirmBillNumber();
    return { billNumber, date: today, client, rows: filled, gstType: gstData.gstType, gstRate: gstData.gstRate };
  };
    ...
    <div className="min-h-screen bg-gray-100 w-full overflow-x-hidden">
      ...
      <header className="bg-gray-900 text-white">
        <div className="max-w-5xl mx-auto w-full px-3 sm:px-4 py-4 flex flex-col sm:flex-row justify-between gap-3">
          <div>
            <div className="text-xs text-gray-400 uppercase">Billing System</div>
            <div className="font-bold text-lg">{COMPANY.name}</div>
            <div className="text-xs text-gray-400 break-words">GSTIN: {COMPANY.gstin}</div>
          </div>
          <div className="text-left sm:text-right">
            <div className="text-xs text-gray-400 uppercase">Next Invoice</div>
            <div className="font-bold text-orange-400 text-xl sm:text-2xl">{getNextBillNumber()}</div>
            ...
            {[
              ...
              <button
                ...
                className={`px-4 py-3 text-sm whitespace-nowrap border-b-2 ${
                  tab === t.id ? 'border-orange-400 text-orange-400' : 'border-transparent text-gray-400'
                }`}
              >{t.label}</button>
            ))}
          </div>
          ...
          <div className="bg-white border p-4">
            <div className="flex flex-col sm:flex-row justify-between gap-2 mb-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowClientPicker(true)} className="border px-3 py-2 text-xs flex items-center gap-1 w-full sm:w-auto">
                  <Search size={12} /> Select
                </button>
                {client && <button onClick={() => setClient(null)}><X size={16} /></button>}
              </div>
            </div>

            {client ? (
              <div className="border p-3 text-sm">
                <div className="font-bold break-words">{client.company || client.name}</div>
                ...
                <div className="flex flex-wrap gap-2 text-xs mt-1">
                  ...
                  {client.state && <span className="flex items-center gap-1"><MapPin size={10} />{client.state}</span>}
                </div>
              </div>
            ) : (
              <div className="border-dashed border text-center py-4 text-xs text-gray-400">No client selected</div>
            )}
          </div>
          ...
          <div className="bg-white border p-4">
            <div className="flex flex-col gap-3">
              ...
              <div className="text-sm space-y-1 break-words">
                ...
                <div className="font-bold text-lg">Total: {formatINR(totals.grandTotal)}</div>
              </div>

              <div className="flex flex-col sm:flex-row gap-2">
                <button onClick={handleNewBill} className="border px-4 py-3 text-sm w-full">
                  Clear
                </button>
                <button
                  onClick={handleGenerateGST}
                  className="bg-black text-white px-4 py-3 text-sm w-full font-semibold hover:bg-gray-800"
                >
                  Generate GST Invoice
                </button>
                <button
                  onClick={handleGenerateNonGST}
                  className="border-2 border-black text-black px-4 py-3 text-sm w-full font-semibold hover:bg-black hover:text-white transition-colors"
                >
        ...
        <div className="fixed inset-0 bg-black/70 z-50 overflow-y-auto">
          <div className="min-h-screen flex items-center justify-center px-2">
            <div className="bg-white w-full max-w-2xl p-3 sm:p-4 relative">
              <button onClick={() => setShowClientPicker(false)} className="absolute top-2 right-2 text-gray-500 hover:text-black">✕</button>
              <ClientManager
                selectorMode
                onSelect={(c) => { setClient(c); setShowClientPicker(false); }}
                onClose={() => setShowClientPicker(false)}
